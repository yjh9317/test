...
void UPlayerInventoryWidget::CreateInventoryPanel(EInventoryPanel Panel)
{
	...
	// else if (LocalSize <= 0)
	// {
	// 	DebugHeader::LogWarning(TEXT("CreateInventoryPanel: Invalid size %d for panel %d"), 
	// 		LocalSize, static_cast<int32>(Panel));
	// 	return;
	// }
...
void UPlayerInventoryWidget::RefreshInventory(EInventoryPanel Panel)
{
	if (!InventoryComponent)
	{
		DebugHeader::LogError(TEXT("RefreshInventory: InventoryComponent is null"));
		return;
	}

	if (!InventoryComponent->PanelsToUse.Contains(Panel))
	{
		DebugHeader::LogWarning(TEXT("RefreshInventory: Panel %d is not in use"), 
			static_cast<int32>(Panel));
		return;
	}
