	...
	AActor* InOwningPlayer, TArray<FItemData>& OutSecuredMaterials)
{
	...
	for(const auto& Material : Materials)
	{
		if(const UDataTable* DT = Material.TableAndRow.DataTable)
		{
			...
			{
				if(FoundItemData.Quantity >= Material.Quantity)
				{
					FoundItemData.Quantity = Material.Quantity;
...
void UStationManagerComponent::ReturnSecuredMaterials(const FCraftingData& InCraftingData, AActor* InOwningPlayer)
{
	...
	for(const auto& Material : LocalMaterialsArray)
	{
		if(const UDataTable* DT = Material.TableAndRow.DataTable)
		{
			FItemData* FoundItemDataFromDT = DT->FindRow<FItemData>(Material.TableAndRow.RowName, "");
			if(!FoundItemDataFromDT) break;

			FoundItemDataFromDT->Quantity = Material.Quantity * InCraftingData.CraftingQuantity;

			TArray<FItemData> LocalInventory;
			int32 LocalSize = 0;
			EInventoryPanel LocalPanel = UInventoryUtilityLibrary::GetInventoryPanelFromItem(*FoundItemDataFromDT);
			LocalInventoryComponent->GetInventoryAndSize(LocalPanel,LocalInventory,LocalSize);

			int32 LocalSlotIndex = 0;
			if(UInventoryUtilityLibrary::HasPartialStack(LocalInventory,*FoundItemDataFromDT,LocalSlotIndex))
			{
				LocalInventoryComponent->AddToStackInInventory(*FoundItemDataFromDT,LocalSlotIndex);
			}
			else
			{
				LocalInventoryComponent->GetEmptyInventorySlot(*FoundItemDataFromDT,LocalSlotIndex);
				LocalInventoryComponent->AddItemToInventoryArray(*FoundItemDataFromDT,LocalSlotIndex);
			}
		}
	}
}
