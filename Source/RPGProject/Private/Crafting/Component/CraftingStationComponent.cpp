...
void UCraftingStationComponent::EventTryToCraftCurrentItem()
{
	...
	UStationsManager* StationsManager = LocalPC->FindComponentByClass<UStationsManager>();
...
void UCraftingStationComponent::StopCraftingProcess(const FCraftingData& InCraftingData, AActor* OwningActor)
{
	...
	UStationsManager* StationsManager = OwningActor->FindComponentByClass<UStationsManager>();
	if(StationsManager)
	{
...
void UCraftingStationComponent::ClearCraftingQueue(AActor* OwningPlayer)
{
	...
	UStationsManager* StationsManager = OwningPlayer->FindComponentByClass<UStationsManager>();
	if(nullptr == StationsManager)
	{
		return;
	}
...
int32 UCraftingStationComponent::GetMaxAmountThatCanBeCrafted(const FCraftingData& InCraftingData, AActor* OwningActor)
{
	...
	TArray<FItemStack> RequiredMaterials;
	UInventoryUtilityLibrary::GetCraftableData(InCraftingData, LocalCraftableItemArray, RequiredMaterials);
	...
	for (const FItemStack& Material : RequiredMaterials)
	{
